...
puedeMovAsi (x,y) c (xf,yf) t@(Tablero lista) = xf >= 0 && xf <5 && yf >= 0 && yf <5 &&  fromMaybe Vacio (piezaAJugador (lista!!(fromIntegral(cordAPos(x,y))))) /= fromMaybe Vacio (piezaAJugador (lista!!( fromIntegral(cordAPos(xf,yf)))))
